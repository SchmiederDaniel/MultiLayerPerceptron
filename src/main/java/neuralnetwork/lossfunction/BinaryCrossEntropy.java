...
public class BinaryCrossEntropy implements LossFunction {
  ...
  public float loss(NumpyArray y_true, NumpyArray y_pred) {
    ...
    for (int rowIndex = 0; rowIndex < y_true.rows(); rowIndex++) {
      for (int colIndex = 0; colIndex < y_true.cols(); colIndex++) {
        sum += -y_true.data[0][rowIndex][colIndex] * Math.log(y_pred.data[0][rowIndex][colIndex])
            - (1 - y_true.data[0][rowIndex][colIndex]) * Math.log(1 - y_pred.data[0][rowIndex][colIndex]);
      }
    }
  ...
  public NumpyArray loss_prime(NumpyArray y_true, NumpyArray y_pred) {
    ...
    for (int rowIndex = 0; rowIndex < y_true.rows(); rowIndex++) {
      for (int colIndex = 0; colIndex < y_true.cols(); colIndex++) {
        newData[rowIndex][colIndex] = ((1f - y_true.data[0][rowIndex][colIndex]) / (1f - y_pred.data[0][rowIndex][colIndex])
            - y_true.data[0][rowIndex][colIndex] / y_pred.data[0][rowIndex][colIndex]) / (y_true.rows() * y_true.cols());
      }
    }
