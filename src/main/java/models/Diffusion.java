...
//      int label = labels[trainIndex];
            ...
            float loss = neuralNetwork.trainSingle(lossFunction, Tensor.of(input2), Tensor.of(target), learningRate);
            lastLosses.add(loss);
...
//      return x;
        ...
        static BufferedImage think(int width, int height) {
            ...
            for (float i = sliderViewSteps.getValue(); i >= 0; i -= thinkStepSize) {
                ...
                float[] output = neuralNetwork.predictThreadSafe(input).transpose().toFlatArray();
