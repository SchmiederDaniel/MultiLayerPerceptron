...
public class AutoEncoderVisualization extends JFrame {
    static List<int[]> imageList;
    static List<Integer> labelList;
    
    float learningRate = 0.00001f;
    private final static LossFunction decoderLossFunction = new BinaryCrossEntropy();
...
//        .activation.tanh()
    ...
    public AutoEncoderVisualization() {
...
//        private final static LossFunction encoderLossFunction = new AbsoluteLoss();
        ...
        void test() {
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                Tensor encoderOutput = encoder.predictThreadSafe(Tensor.of(x));
                Tensor decoderOutput = decoder.predictThreadSafe(encoderOutput);
        ...
        void train() {
            ...
            Tensor encoderOutput = encoder.predict(Tensor.of(x));
            Tensor grad = decoder.trainSingleGrad(decoderLossFunction, encoderOutput, Tensor.of(x), learningRate);
            
            encoder.backpropagate(grad, learningRate);
...
//            neuralNetworkEncoder.trainWithoutPredict(encoderLossFunction, encoderOutput, grad, learningRate * 0.5f);
        ...
        public void paint(Graphics graphics) {
            ...
            Tensor x = encoder.predictThreadSafe(inputs);
            ...
            float[] y = decoder.predictThreadSafe(x).transpose().toFlatArray();
    ...
    static class TwoDimVisualizer extends JPanel {
        ...
        public void paint(Graphics graphics) {
            ...
            for (int i = 0; i < 200; i++) {
                ...
                float[] inputs = pixelsToFloat(pixels);
                Tensor output = encoder.predictThreadSafe(inputs);
                
                float x = output.toFlatArray()[0];
                float y = output.toFlatArray()[1];
