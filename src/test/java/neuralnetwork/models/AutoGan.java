  ...
  class Scene extends JPanel {
    ...
    void test() {
      ...
      for (int i = 0; i < 200; i++) {
        ...
        float[] encoded = encoder.predict(NumpyArray.of(realData)).transpose().data[0][0];
        float[] decoded = decoder.predict(NumpyArray.of(encoded)).transpose().data[0][0];
        ...
        float discriminated = discriminator.predict(NumpyArray.of(combined)).data[0][0][0];
        ...
        System.arraycopy(order ? realData : decoded, 0, combined, 0, imageResolution * imageResolution);
        System.arraycopy(order ? decoded : realData, 0, combined, imageResolution * imageResolution, imageResolution * imageResolution);
        discriminated = discriminator.predict(NumpyArray.of(combined)).data[0][0][0];
...
//      int label = labels[trainIndex];
      ...
      { // train generator
        ...
        float[] data = grad.transpose().data[0][0];
    ...
    public void paint(Graphics graphics) {
      ...
      float[] encoded = encoder.predictThreadSafe(encoderInput).transpose().data[0][0];
      float[] decoded = decoder.predictThreadSafe(encoded).transpose().data[0][0];
