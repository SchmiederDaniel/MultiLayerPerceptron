...
//      return x;
    ...
    static BufferedImage think(int width, int height) {
      ...
      for (float i = sliderViewSteps.getValue(); i >= 0; i -= thinkStepSize) {
        ...
        float[] output = neuralNetwork.predictThreadSafe(input).transpose().data[0][0];
      ...
      float[] difference = NumpyArray.of(noisyOriginal).subtract(NumpyArray.of(noisy)).add(NumpyArray.of(0.5f)).transpose().data[0][0];
